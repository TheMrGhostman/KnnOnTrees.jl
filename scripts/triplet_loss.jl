...
using Flux, Zygote, Mill, KnnOnTrees #, GHMill 
...
metric = reflectmetric(train[1][1]; weight_sampler=randn, weight_transform=softplus);
...
for iter ∈ tqdm(1:iters)
    xₐ, xₚ, xₙ = SampleTriplets(train..., batch_size, true);
    # Gradients ≈ Forward + Backward
    loss_, grad = Flux.withgradient(() -> triplet_loss(metric, xₐ, xₚ, xₙ, 0), ps);
    # Optimization step
    Flux.update!(opt, ps, grad)
    # Logging training
    acc_ = triplet_accuracy(metric, xₐ, xₚ, xₙ) |> Flux.mean
    if mod(iter, 20)==0
...
results = (model = metric, seed=seed,params=ps, iters=iters, learning_rate=learning_rate, batch_size=batch_size, history=history, train=train, val=val, test=test, ui=id[:ui])
